...
**Regras de negócio:**
- A indisponibilidade do Serviço de Consolidação não deve gerar perda de eventos — o broker retém as mensagens até o serviço estar disponível novamente.
...
O Serviço de Consolidação mantém uma tabela local de lançamentos processados. O campo `id` do evento é o UUID gerado pelo Serviço de Lançamentos no momento do registro ([RF-01](#rf-01)) e incluído no payload do evento `LancamentoRegistrado`. Esse UUID é usado como chave primária na tabela local:
...
| <span id="nfr-07"></span>NFR-07 | O Serviço de Consolidação deve proteger-se contra sobrecarga de requisições | Resiliência | Rate limiting na borda da API; excedentes recebem HTTP 429 | [D-04](drivers.md#d-04), [NFR-02](#nfr-02) |
