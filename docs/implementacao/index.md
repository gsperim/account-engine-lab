...
**Total: ~146 testes** — `lancamentos` (~86) + `consolidado` (~60), todos verdes.
...
**Padrões:** `@Retry` + `@CircuitBreaker` em `OutboxPublisher.publicar()`
...
### Decisão 4 — Reconciliação Diária e Recuperação Catastrófica

**Status:** Implementado — artefatos na seção "Etapa 7 — Entregues" abaixo
...
**Reconciliação diária** — `ReconciliacaoDiariaJob` (`@Scheduled(cron="0 0 2 * * *")`) consulta o serviço de lançamentos via `LancamentosGateway` e valida os totais:
...
Se diverge → corrige + loga + alerta (Prometheus: saldo_reconciliado_divergencias_total)
```

**Recuperação catastrófica** — `POST /admin/reconstruir` (`ReconstruirConsolidadoService`) reconstrói o histórico do consolidado consultando o lançamentos data a data:

```
POST /consolidacao/admin/reconstruir?data_inicio=YYYY-MM-DD&data_fim=YYYY-MM-DD
→ para cada data: busca lançamentos via gateway → recalcula → sobrescreve saldo_consolidado
```

O consolidado é um **read model**: pode ser sempre reconstruído a partir do lançamentos (source of truth), eliminando a dependência de restauração de backup em caso de perda do banco.
...
Todos os itens da Etapa 7 estão implementados, testados e no `develop`. **~146 testes verdes.**
...
| Reconciliação diária | `ReconciliacaoDiariaJob` + `LancamentosGateway` — compara totais às 2h, métrica de divergência |
| Recuperação catastrófica | `POST /admin/reconstruir` — `ReconstruirConsolidadoService`, itera dia a dia via gateway |
...
| Item | Artefatos |
|------|-----------|
| GitHub Actions CI/CD | `ci.yml` — testes + JaCoCo + Trivy + Infracost; `cd-lancamentos.yml` + `cd-consolidado.yml` — CD independente por serviço, SemVer via `VERSION` file, OIDC sem credenciais estáticas |
| Chaos Engineering | 5 experimentos executados com Pumba + k6 — todos os NFRs confirmados; evidências em `docs/implementacao/caos.md` |
| GitHub Pages | `docs.yml` — MkDocs + C4 (structurizr-site-generatr) + relatórios de testes + JaCoCo coverage + Infracost |
