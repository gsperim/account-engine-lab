...
**Etapa:** 7 — Em finalização (2025-05-11)  
...
**Total: 68 testes** — `lancamentos` (40) + `consolidado` (28), todos verdes.
...
## Pendências — Finalização da Etapa 7

> **Ponto de retomada (2025-05-11)**
>
> Os dois serviços estão rodando em produção local com 12 containers. O k6 smoke test passou com **0% de falha** (p95 ≈ 10ms). Recording rules do Prometheus produzindo dados (`success_rate = 1.0` para lancamentos, consolidado e keycloak). Dashboard **Plataforma** 100% funcional (Uptime, PostgreSQL, Redis, RabbitMQ, Traefik). Dashboards **Infraestrutura** e **Negócio** parcialmente funcionais — painéis de latência e lançamentos precisam de tráfego k6 para popular; painel Taxa de Erro precisa de `or vector(0)` para não mostrar "No data" quando não há erros 5xx.
>
> **Próximos passos sugeridos (em ordem):**
> 1. Fix pontual: `or vector(0)` no painel Taxa de Erro (infra.json) — 5 minutos
> 2. Rodar `k6 run tests/k6/smoke.js` por 2 minutos para popular painéis de latência e negócio
> 3. README (bloqueador para entrega)
> 4. Publicar repositório no GitHub (bloqueador para entrega)
> 5. JWT/Spring Security
> 6. Outbox cleanup + DLQ consumer

Itens organizados por prioridade de impacto no desafio.

---

### 🔴 Eliminatórios (requisitos obrigatórios do desafio)

#### README desatualizado

O `README.md` raiz ainda descreve o projeto como *"fase de arquitetura"*. Precisa ser reescrito com instruções completas de execução local, incluindo pré-requisitos, `setup.sh`, URLs de acesso e comandos k6.

#### Repositório público no GitHub

O repositório existe apenas localmente. O desafio exige publicação em repositório público. Inclui a decisão sobre o que vai e o que não vai para o GitHub (`.env`, certificados, tokens).

---

### 🟡 Lacunas técnicas relevantes

#### JWT / Spring Security — endpoints abertos

A spec OpenAPI define `bearerAuth: []` em todos os endpoints de negócio. Nenhum dos serviços tem `spring-boot-starter-oauth2-resource-server` nem `SecurityFilterChain` configurado. O Keycloak está rodando mas nenhum serviço valida token JWT. O `operadorId` é extraído do header `X-Operator-Id` em texto puro — sem identidade verificada.

**Decisão pendente:** implementar validação JWT stateless via JWKS do Keycloak, extraindo `sub` como `operadorId`.

#### Outbox — cleanup job ausente

A tabela `outbox` cresce indefinidamente. A estratégia de retenção (7 dias, lote de 100, `@Scheduled`) foi documentada em [arquitetura/dados](../arquitetura/dados.md) mas o código não foi criado.

#### DLQ sem consumer

Mensagens que falham no `LancamentoEventoConsumer` vão para `consolidacao.lancamentos.dlq`. Não existe nenhum processo para reprocessá-las — a fila é um cemitério hoje. Precisa de um consumer de DLQ ou de um endpoint admin que dispare o reprocessamento.

#### Grafana dashboards — validação parcial concluída

As métricas foram corrigidas durante a Etapa 7 (nomes Micrometer vs OTEL, label `job` vs `service`, `outcome` vs `http_status_code`). Estado atual por dashboard:

| Dashboard | Status | Pendência |
|-----------|--------|-----------|
| Plataforma | ✅ 100% funcional | — |
| Logs Centralizados | ✅ funcional | — |
| Infraestrutura | 🟡 parcial | Taxa de Erro: `or vector(0)` quando sem erros 5xx; Latência: popular com tráfego k6 |
| Negócio | 🟡 parcial | Lançamentos/minuto e Cache Hit Rate: popular com tráfego k6 |
